...
import torchvision, torchvision.datasets, torchvision.transforms, torch.distributed
import io
import time
import dataclasses
...
class HandwritingNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        # The network from the tutorial uses convolutions... gonna skip that for now to see what I can do.
        self.conv_layer_1 = torch.nn.Conv2d(1, 6, 3)
        self.conv_layer_2 = torch.nn.Conv2d(6, 16, 3)
        # The 28x28 image sampled down into our new form is 5x5, with 16 channels
        self.layer_1 = torch.nn.Linear(5 * 5 * 16, 120)
        self.layer_2 = torch.nn.Linear(120, 84)
        self.layer_3 = torch.nn.Linear(84, 10)

    def forward(self, x):
        res = torch.nn.functional.max_pool2d(
            torch.nn.functional.relu(self.conv_layer_1(x)), 2
        )
        res = torch.nn.functional.max_pool2d(
            torch.nn.functional.relu(self.conv_layer_2(res)), 2
        )

        # The 28x28 image sampled down into our new form is 5x5, with 16 channels
        res = res.view(-1, 5 * 5 * 16)
        res = torch.nn.functional.relu(self.layer_1(res))
        res = torch.nn.functional.relu(self.layer_2(res))
...
def get_gcp_dataset(bucket_name: str, stats_collector: StatsCollector):
    ...
    return gcp_loader.Bucket(
        ...
        transform=torchvision.transforms.Compose(
            [lambda data: Image.open(io.BytesIO(data)), get_mnist_transforms()],
        ),
...
# def get_training_partition(
#     world_size: int, rank: int, seed: int, dataset: torch.utils.data.Dataset
# ) -> List[int]:
#     random_gen = random.Random(seed)
#     indexes = list(range(len(dataset)))
#     random.shuffle(indexes, random_gen.random)
#     num_samples_for_node = len(dataset) // world_size
#     slice_start = num_samples_for_node * rank
#     # Handle the case of not being evenly divisible by throwing the extras on the highest ranking node
#     if rank == world_size - 1:
#         return indexes[slice_start:]

#     return indexes[slice_start : slice_start + num_samples_for_node]
...
def main(*, world_size, rank, addr, port, seed, batch_size, fetch_size, min_queue_size):
    ...
    cloud_data = get_gcp_dataset("modeling-distributed-trainig-mnist", stats_collector)
    ...
    # A circular dependency... but alas
